...
pub fn build() -> String {
    ...
    process_packets("../pumpkin-protocol/src/java/server/play", &mut interface, &mut serverbound_variant);
    // Process clientbound packets
    process_packets("../pumpkin-protocol/src/java/client/play", &mut interface, &mut clientbound_variant);
...
fn parse_packet_file(path: &Path, interface: &mut Interface, variant: &mut Variant) {
    ...
    for item in file.items {
        if let Item::Struct(s) = item {
            ...
            if !s.attrs.iter().any(|attr| attr.path().is_ident("java_packet")) {
                continue;
            }
